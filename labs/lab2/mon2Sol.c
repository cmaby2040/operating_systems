...
int main(int argc, char **argv)
{
    ...
    {
        execl(program, program, NULL);     /* excute the process that we are going to monitor */
	      exit(-1); /* in case execl fails */
    }

    /* Second step: Create the pipe to be used for connecting procmon to filter */
    if (pipe(pipefd) == -1) /* retur n the two integrers  
    {
    ...
    { /* In the child process to run filter */
            ...
            if(dup2(pipefd[0], 0) == -1) { perror("dup2: "); exit(-1); }    /* here in input is connected to the pipe    dup works connect the ouput of the pipe to the connector of the recieiver*/
	          close(pipefd[0]); /* no longer need these file descriptors */
            close(pipefd[1]);     /*always close the pipe here on the other side   */
    ...
    /* Fourth step: Lets create the procmon process - don't forget to connect to the pipe */
     ...
     { /* start procmon but first redirect the standard output to the pipe */
         dup2(pipefd[1], 1);  /*this the */
	      close(pipefd[0]);     /* here you are killing the ones that came from the parent*/
         close(pipefd[1]);
    ...
    close(pipefd[0]);      /*this is killing the original pipe connection here form the parent so that it doesnt recieve any of these things*/
    close(pipefd[1]);
