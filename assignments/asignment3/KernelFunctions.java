public class KernelFunctions
{
	...
	public static void pageReplAlgorithmCLOCK(int vpage, Process prc){
		...
		while(whileTest==true) { 
			...
			if(prc.pageTable[vPageReplaced].used==true) { // check used bit 
	...
	public static void pageReplAlgorithmLRU(int vpage, Process prc){
		...
		for (int i=0; i < prc.pageTable.length; i++){
			if (prc.pageTable[i].valid){
				...
				// check time each to make replacement saving the lowest value.
				if (prc.pageTable[i].tmStamp < prc.pageTable[vPageReplaced].tmStamp){
					vPageReplaced = i; 
				}
		...
		prc.pageTable[vpage].frameNum = prc.pageTable[vPageReplaced].frameNum; 
		prc.pageTable[vpage].valid = true;          
	}
